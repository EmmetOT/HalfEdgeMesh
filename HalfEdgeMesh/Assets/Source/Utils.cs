...
namespace HalfEdge {
    public static class Utils {
        private static readonly Vector3[] m_trianglePoints;
        private static readonly Vector3[] m_circlePoints;

        private static readonly RaycastHit[] m_nonAllocClampRaycastResults = new RaycastHit[5];
        ...
        static Utils() {
            m_rainbowGradient = CreateGradient(Color.red, ToColor(0xFF8200), Color.yellow, Color.green, ToColor(0x005DFF), ToColor(0xAE00FF), ToColor(0xFF00CB));

            m_horizontalPlane = new Plane(Vector3.up, Vector3.zero);
            m_verticalPlane = new Plane(Vector3.back, Vector3.zero);

            m_trianglePoints = new Vector3[] { new Vector3(-0.333f, 0f, -0.333f), new Vector3(0f, 0f, 0.333f), new Vector3(0.333f, 0f, -0.333f) };
        ...
        public static RenderTexture CreateTempRenderTexture(int width,
                int height,
                Color? col = null,
                UnityEngine.Experimental.Rendering.GraphicsFormat? format = null,
                int antiAliasing = 1) {
        ...
        public static (int x, int y, int z, int w) PiecewiseOp(this Vector4 input, Func<float, int> function) =>
                (function(input.x), function(input.y), function(input.z), function(input.w));
        ...
        public static (int x, int y, int z) PiecewiseOp(this (int inX, int inY, int inZ) input, Func<int, int> function) =>
                (function(input.inX), function(input.inY), function(input.inZ));
        ...
        public static Vector4 PiecewiseOp(this Vector4 input, Func<float, float> function) =>
                new Vector4(function(input.x), function(input.y), function(input.z), function(input.w));
        ...
        public static Vector3 CalculateAvoidanceDirection(Vector3 currentPosition,
        ...
        public static float DistanceToTriangle(Vector3 p, Vector3 a, Vector3 b, Vector3 c) {
            ...
            return Mathf.Sqrt(
                    (Mathf.Sign(Vector3.Dot(Vector3.Cross(ba, normal), pa)) +
                     ...
                     2f)
                            ? Mathf.Min(
                                    Mathf.Min(Dot2(ba * Mathf.Clamp01(Vector3.Dot(ba, pa) / Dot2(ba)) - pa), Dot2(cb * Mathf.Clamp01(Vector3.Dot(cb, pb) / Dot2(cb)) - pb)),
        ...
        private static Vector3 InterpolateSurfacePosition(Vector3 p, Vector3 v_0, Vector3 v_1, Vector3 v_2, Vector3 n_0, Vector3 n_1, Vector3 n_2) {
            ...
            return (v_0 * u) + (v_1 * v) + (v_2 * w) - (c1 * u * v) - (c2 * v * w) - (c3 * u * w);
        }
        ...
        public static void SortAnticlockwise(Vector3 normal,
                Vector3 a,
                Vector3 b,
                Vector3 c,
                int a_index,
                int b_index,
                int c_index,
                out Vector3 a_sorted,
                out Vector3 b_sorted,
                out Vector3 c_sorted,
                out int a_index_sorted,
                out int b_index_sorted,
                out int c_index_sorted) {
        ...
        public static bool TryIntersect(this Plane plane, Vector3 lineSegmentStart, Vector3 lineSegmentEnd, out Vector3 intersectionPoint, out float t) {
            ...
            var lineDirection = lineSegmentEnd - lineSegmentStart;

            // Calculate the distances from the line start and end points to the plane
            var startDistance = plane.GetDistanceToPoint(lineSegmentStart);
            var endDistance = plane.GetDistanceToPoint(lineSegmentEnd);

            // Check if the line segment is parallel to the plane
            if (Mathf.Approximately(startDistance, endDistance)) {
                return false; // Line segment is parallel to the plane
            }

            // Calculate the fraction along the line segment where the intersection occurs
        ...
        public static IEnumerable<MethodInfo> GetMethodsWithAttribute(this Type classType, Type attributeType) {
            return classType.GetMethods(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.DeclaredOnly)
